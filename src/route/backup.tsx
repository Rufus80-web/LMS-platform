...
const ExercisePage: React.FC = () => {
  ...
  const [_selectedLanguage, setSelectedLanguage] =
  ...
  const handleSubmit = async () => {
    ...
    const fullAnswers = exam.exercises.map((ex: Exercise, idx: number) =>
      ex.type === "programming" ? codeAnswers[idx] : answers[idx]
    );
  ...
  return (
    <div className="p-4 space-y-6 bg-[#222222]">
      {getAccessExam() && (
        <ExamTimer
      ...
      {exam.exercises.map((ex: Exercise, idx: number) => (
        <React.Fragment>
          {getAccessExam() && (
            <div key={idx} className="bg-[#33333353] text-white shadow rounded p-4">
              <p>
              ...
              {exam?.type === "programming" && (
                <CodeEditor
                  ...
                  onMount={onMount}
                  onChange={handleProgremmingLangChange}
                  codeFromEditor={codeAnswers[idx]}
                  index={idx}
                  getEditorValue={() => handleCodeChange(idx, codeAnswers[idx])}
                />
              )}
            </div>
          )}
